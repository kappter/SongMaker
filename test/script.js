...
function copyRiffusionPrompt() {
  const blocks = Array.from(timeline.children);
  if (blocks.length === 0) {
    alert('No song blocks available to generate a prompt.');
    return;
  }

  // Get song-wide properties (use the first block for consistency)
  const firstBlock = blocks[0];
  const rootNote = firstBlock.getAttribute('data-root-note');
  const mode = firstBlock.getAttribute('data-mode');
  const tempo = firstBlock.getAttribute('data-tempo');
  const timeSignature = firstBlock.getAttribute('data-time-signature');

  // Start building the prompt
  let prompt = `Create a progressive, atmospheric track titled "${currentSongName}" in ${rootNote} ${mode}, ${tempo} BPM, ${timeSignature} time signature. `;

  // Map block types to descriptions
  const blockDescriptions = {
    intro: {
      text: 'Begin with a mysterious and spacey intro that feels atmospheric and ambient.',
      feelMap: {
        Atmospheric: 'with ethereal pads and subtle drones',
        Mystical: 'with shimmering arpeggios and soft chimes',
        Calmness: 'with gentle, flowing textures'
      }
    },
    verse: {
      text: 'Transition into energetic verses with themes of rebellion and triumph, using expressive melodies and syncopated rhythms.',
      feelMap: {
        Rebellion: 'with gritty guitar riffs and driving percussion',
        Triumph: 'with uplifting chord progressions and bold dynamics',
        Intense: 'with sharp, staccato synth leads'
      }
    },
    chorus: {
      text: 'Introduce a catchy, upbeat chorus with a joyful, repetitive vocal phrase.',
      feelMap: {
        Happiness: 'featuring bright, major-key melodies',
        Euphoria: 'with soaring vocal harmonies and lush pads',
        Bliss: 'with warm, enveloping textures'
      }
    },
    bridge: {
      text: 'Include a bridge section that is dreamy and trippy, evoking echoes of forgotten dreams with reverb-heavy textures.',
      feelMap: {
        Trippy: 'with swirling, psychedelic effects',
        Mystical: 'with deep, resonant synth layers',
        Awakening: 'with gradually building intensity'
      }
    },
    outro: {
      text: 'End with a conclusive, emotional outro that brings resolution, tying together the song’s themes.',
      feelMap: {
        Resolution: 'with fading, reflective melodies',
        Calmness: 'with soft, resolving chords',
        Triumph: 'with a grand, final cadence'
      }
    }
  };

  // Build block-specific prompt parts
  blocks.forEach((block, index) => {
    const type = block.classList[1];
    const feel = block.getAttribute('data-feel');
    const lyrics = block.getAttribute('data-lyrics');

    if (blockDescriptions[type]) {
      let blockText = blockDescriptions[type].text;
      if (type === 'chorus' && lyrics) {
        blockText = blockText.replace('repetitive vocal phrase.', `repetitive vocal phrase ("${truncateLyrics(lyrics)}").`);
      }
      if (blockDescriptions[type].feelMap[feel]) {
        blockText += ` ${blockDescriptions[type].feelMap[feel]}`;
      }
      prompt += blockText + ' ';
    }
  });

  // Add general instrumentation
  ...
  navigator.clipboard.writeText(prompt).then(() => {
    alert('Riffusion prompt copied to clipboard!');
  }).catch(err => {
