...
async function loadAudio() {
    buffers.kick = await fetch('tick.wav')
        .then(response => response.arrayBuffer())
        .then(arrayBuffer => audioContext.decodeAudioData(arrayBuffer));
    buffers.snare = await fetch('tock.wav')
        .then(response => response.arrayBuffer())
        .then(arrayBuffer => audioContext.decodeAudioData(arrayBuffer));
}
...
function scheduleStep() {
    ...
    const stepInterval = 0.1; // 100ms = 0.1s
    while (nextStepTime < audioContext.currentTime + 0.1) { // Look ahead 100ms
        const stepData = songData[currentStep];
        if (stepData) {
            if (stepData.kick) playSound('kick', nextStepTime);
            if (stepData.snare) playSound('snare', nextStepTime);
        }

        currentStep = (currentStep + 1) % songData.length;
...
function startPlayback() {
    if (!isPlaying) {
        isPlaying = true;
        nextStepTime = audioContext.currentTime + 0.1; // Start slightly in future
        scheduleStep();
    }
}

// Load song and initialize
...
document.getElementById('playButton').addEventListener('click', () => {
    if (Object.keys(buffers).length === 0) {
        loadAudio().then(startPlayback); // Load audio first time
    } else {
        startPlayback();
    }
});
...
document.getElementById('loadButton').addEventListener('click', () => {
    ...
    fetch(`${songName}.js`)
        ...
        .then(text => {
            const song = eval(text); // Note: eval is risky; consider safer parsing
            loadSong(song);
        });
});

// Initialize grid (unchanged from your original)
function updateGrid() {
    ...
    instruments.forEach(instrument => {
        ...
        row - instrument name mapping here
        row.appendChild(label);
        songData.forEach((step, index) => {
            const cell = document.createElement('div');
            cell.className = 'cell';
            if (step[instrument]) cell.classList.add('active');
...
function updateGridHighlight() {
    ...
    cells.forEach((cell, index) => {
        ...
        if (index % songData.length === currentStep) {
